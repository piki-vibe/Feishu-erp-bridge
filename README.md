...
## 🧾 发票 OCR 工作台（2026-03-28 更新留痕）

### 本次更新内容
- 新增独立的“发票 OCR”工作台页面，与主任务页分离展示
- OCR 工作台容器改为独立铺满布局，接口示例默认折叠，减少页面底部留白
- 识别结果卡片支持展开查看 `total` 全量信息，便于联调、审计与误判排查
- OCR 子服务支持单独启动、关闭和状态查看，空闲时保持低功耗
- 保留原公开提取接口契约，请求方式和参数不变
- OCR 服务入口默认启用 `4` 个线程处理并发请求，识别阶段仍走隔离子进程，兼顾吞吐与低常驻内存
- 识别结果聚焦为“发票号码”，且只在 OCR 文本里命中 `发票号码` 标签时才提取后续号码
- 支持格式扩展为：`PDF`、`JPG`、`JPEG`、`PNG`、`BMP`、`WEBP`
- 非上述格式统一返回：`文件格式不对，仅支持 PDF 和图片`
- 若未命中 `发票号码` 标签，接口统一返回：`无发票号码`

### OCR 服务控制

前端工作台会调用以下受登录保护的管理接口：

| 接口 | 说明 |
|------|------|
| `GET /api/ocr/service/status` | 查看 OCR 子服务状态 |
| `POST /api/ocr/service/start` | 启动 OCR 子服务 |
| `POST /api/ocr/service/stop` | 关闭 OCR 子服务 |

### 公开识别接口

以下识别接口继续保持公开使用，未修改原请求方式与参数名：

| 接口 | 请求方式 | 参数 |
|------|----------|------|
| `/api/extract` | `POST` | `multipart/form-data` 的 `file`，或 JSON 的 `url` |
| `/api/extract-batch` | `POST` | `multipart/form-data` 的 `files`，或 JSON 的 `urls` |

单文件返回在原有 `invoice_number`、`processing_time` 基础上新增：

- `total`：完整 OCR 提取信息与严格标签匹配明细

### 已完成的稳定性调整
- OCR 子服务改为按需拉起工作进程，降低长期占用
- 发票 OCR 页面改为按需加载，减轻主工作台首屏渲染压力
- OCR 页面状态轮询改为低频可见态刷新，减少空闲标签页的无效开销
- 低频功能面板已拆为独立前端分包，生产构建主入口进一步降到约 `102 kB`
- 开发环境代理日志默认静默，降低轮询与调试流量带来的终端刷屏和卡顿
- 图片识别链路补齐 Windows 编码处理，避免 `gbk` 输出异常
- OCR 启动/关闭脚本统一改为 UTF-8 输出，避免中文路径在服务控制接口中乱码
- 后端账户文件写入增加串行安全写，降低登录/资料更新时的读写竞争风险
- 后端实例记录文件也已切换到串行安全写，降低高并发状态更新时的文件损坏风险

### 本轮回归验证（2026-03-28）
- 有效 PDF 样本：成功返回发票号码 `26952000000276075601`
- 无“发票号码”图片样本：成功返回 `无发票号码`
- 非法格式样本：成功返回 `文件格式不对，仅支持 PDF 和图片`
- OCR 服务控制接口：`status / stop / start` 已完成一轮实机回归
- 主后端重启后，完成 `8` 组并发登录 + `8` 组并发资料更新压测，`16/16` 请求返回 `200`
